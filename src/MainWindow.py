...
class MainWindowWidget(QtGui.QMainWindow):
    def __init__(self):
        ...
        self.fileWidget.setMinimumWidth(100)
        self.fileWidget.setMaximumWidth(500)
    ...
    def getQUANQUAL_table(self):
        if len(self.MSRT) >= 1 and len(self.files) >= 1:
            ...
            for i in range(1, len(self.MSRT['rt'])+1):
                ab[str(i)] = area[:, i-1].T
            np.savetxt('QUAN_table.txt', ab, delimiter=';', fmt="%10s")

            # build QUAL Table
            # ff = open('QUAL_table.msp', 'w')
            # ddtype = [('0', 'S10'), ('1', float)]
            # for i in range(2, self.MSRT['ms'].shape[1]+2)
            # ddtype2 = np.zeros((self.MSRT['ms'].shape[1], 2))
            # for i in range(0, self.MSRT['ms'].shape[1]):
            #     ddtype2[i, :] = [(str(i)+'0', float), (str(i)+'1', float)]
